...
import { Button, Col, Input, Preloader, Row } from 'react-materialize';
...
class DetalleVuelo extends Component {
  ...
  guardarVuelo = () => {
    ...
    if (numErrores > 0) {
      ...
      const vuelo = {
        IDVuelo, matriculaID,
        fechaSalida: new Date(fechaSalida).toJSON(),
        fechaLlegada: new Date(fechaLlegada).toJSON(),
        origen, destino, ruta, idEstado
      };
      if (this.props.match.params.id) {
  ...
  mostrarError = () => {
    ...
    return (<h3 className="red-text">{ this.props.error }</h3>);
  };
